...
public class Tax1 {
	public static void main(String[]args){
		...
		(int)(compuT(filer, inc) * 100) / 100.0);
	}
	
	public static double compuT(int filer, double inc){
		...
		if (inc <= brackets[filer][0])
			return tax = inc * rates[0]; // Done
		else
			tax = brackets[filer][0] * rates[0];
		
		//checks for brackets 2, 3, 4, 5
		for (int i = 1; i < brackets[0].length; i++){
			if (income > brackets[filer][i])
				tax += (brackets[filer][i] - brackets[filer][i - 1]) * 
						rates[i];
			else{
				tax += (income - brackets[filer][i - 1]) * rates[i];
				return tax; // Done
				}
			}
			...
			return tax += (income -brackets[filer][4] * rates[5]);
	}
}
